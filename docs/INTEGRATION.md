...
OronBox Package Network owns the userspace IP gateway only. The host remains responsible for the Xiaomi transport and device lifecycle

## Start

1. Authenticate the Xiaomi device and establish its normal protocol transport
2. Open one `OronboxNetworkSession` for the connected device
...
```dart
final network = await OronboxNetworkSession.open(
  config: OronboxNetworkConfig(
