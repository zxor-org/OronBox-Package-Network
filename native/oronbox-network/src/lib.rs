...
static RUNTIME: Lazy<Runtime> = Lazy::new(|| {
    Builder::new_multi_thread()
        .thread_name("oronbox-network")
        .enable_all()
        .build()
        .expect("create OronBox network runtime")
});
...
#[unsafe(no_mangle)]
pub extern "C" fn ob_network_abi_version() -> u32 {
    ABI_VERSION
}
...
pub unsafe extern "C" fn ob_network_push(handle: u64, data: *const u8, length: usize) -> i32 {
    ffi_result(|| {
...
pub unsafe extern "C" fn ob_network_event_peek(
    handle: u64,
    out_kind: *mut u32,
...
/// bytes.
pub unsafe extern "C" fn ob_network_event_read(
    handle: u64,
    buffer: *mut u8,
    capacity: usize,
    out_kind: *mut u32,
...
pub unsafe extern "C" fn ob_network_get_snapshot(
    handle: u64,
...
#[unsafe(no_mangle)]
pub extern "C" fn ob_network_close(handle: u64) -> i32 {
    ffi_result(|| {
...
#[unsafe(no_mangle)]
pub extern "C" fn ob_network_last_error() -> *const c_char {
...
mod tests {
    ...
    fn exports_current_abi_version() {
        assert_eq!(ob_network_abi_version(), ABI_VERSION);
    }
    ...
    fn opens_snapshots_and_closes_a_session() {
        let mut handle = 0;
        assert_eq!(
            unsafe { ob_network_open(&config(), Some(wake), &mut handle) },
            ZB_OK
        );
        assert_ne!(handle, 0);
        ...
        assert_eq!(
            unsafe { ob_network_get_snapshot(handle, &mut snapshot) },
            ZB_OK
        );
